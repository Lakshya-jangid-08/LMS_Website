...
function Navbar() {
  ...
  return (
    <div className={`w-full items-center flex justify-between border-b-2 border-black 
      sm:px-10 md:px-14 lg:px-10 py-4  ${isCourseList ? 'bg-white' : 'bg-cyan-100/70'}`}>
        ...
        {user && <>
          <div className="">
            <button onClick={()=>{navigate('/educator') 
                                  console.log('clicked educator')
             } } className='text-black'>
              {isEducator ? 'Educator Dashboard ' : 'Become Educator'}</button> |
            <Link className='text-black' to='/my-enrollments' onClick={()=>{console.log('clicked my enrollments');
            }}> My Enrollments</Link>
          </div>
        </>}
        {user ? <UserButton /> :
