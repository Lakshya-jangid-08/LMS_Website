...
function CourseDetails() {
  ...
  const { allCourses, calculateRating, calculateChapterTime,
    calculateCourseTime, numberOfLectures, currency } = useData();
    ...
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='md:text-course-details-heading-large text-course-details-heading-small font-semibold text-black'>{courseData.courseTitle}</h1>
          ...
          <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            {/* {console.log(courseData.educatorname)} */}
            <p className='text-blue-600'> ({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'})</p>
            <p >{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'Students' : 'Student'}</p>
          </div>
          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Lakshya</span></p>
        ...
        <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          {
            PlayerData ? <YouTube videoId={'watch?v=tucWbkH5WX0&list=RDtucWbkH5WX0&start_radio=1'} opts={{ playerVars: { autoplay: 1 } }} iframClassName='w-full aspect-video' />
